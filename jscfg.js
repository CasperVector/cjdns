...
var deps = {}, x_cppflags = {}, testlinks = [],
    ...
    env = { CC: 'cc', O_FLAGS: ['-O2', '-std=c99'], X_CFLAGS: ['-pedantic'],
        X_CPPFLAGS: ['-I.'], CPPFLAGS: ['-D_FORTIFY_SOURCE=2',
            '-D' + jscfg.systemName + '=1', '-DNumberCompress_TYPE=v3x5x8',
            '-DIdentity_CHECK=1', '-DAllocator_USE_CANARIES=1', '-DPARANOIA=1',
            '-DLog_INFO', '-DCJD_PACKAGE_VERSION="unknown"'] };
...
var cpp = function (filename) {
    ...
    var proc = child_process.spawnSync(
        env.CC, ['-E'].concat(env.O_FLAGS).concat(env.CPPFLAGS)
            .concat(x_cppflags[filename] || env.X_CPPFLAGS)
            .concat([filename]), { encoding: 'utf-8' }
    );
    if (proc.status !== 0) throw new Error(proc.stderr);
    var js, cpped = ext_sub(filename, '.ii');
    try { js = fs.readFileSync(cpped, 'utf-8'); }
    catch (e) { /* nop */ }
    if (proc.stdout !== js) fs.writeFileSync(cpped, proc.stdout);
...
var fmt_mk = function () {
    ...
    var ret = 'CC = ' + env.CC + '\nO_FLAGS = ' + maybe_quote(env.O_FLAGS) +
        '\nCPPFLAGS = ' + maybe_quote(env.CPPFLAGS) + '\nX_CFLAGS = ' +
        maybe_quote(env.X_CFLAGS) + '\nX_CPPFLAGS = ' +
...
var main = function (argv) {
    ...
    x_cppflags[signc] = ['-I.', '-I./crypto/sign'],
